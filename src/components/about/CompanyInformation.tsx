...
type CompanyInformationSection = NonNullable<PageAboutQueryResult>["companyInformation"];
...
export const CompanyInformation = ({ data }: { data: CompanyInformationSection }) => {
    if (!data) return null;
    ...
    return (
        <section className="relative w-full py-16 lg:py-24 px-4 overflow-hidden">
            ...
            {hexagonNodeIllustration && (
                <div className="absolute inset-0 z-1 pointer-events-none flex items-end justify-center">
            ...
            <div className="container mx-auto px-4 lg:px-8 relative z-10 flex flex-col lg:flex-row items-center gap-10 lg:gap-20">
                {/* Left — Logo */}
                <motion.div
                ...
                <motion.div
                    ...
                    className="flex-1 flex flex-col items-start lg:items-end gap-4 z-10"
                >
                    ...
                    {infoItems.map(({ key, label }) => {
                        ...
                        return (
                            <div key={key} className="flex flex-col items-start lg:items-end">
                                <span className="text-sm lg:text-base font-bold text-white/60 uppercase tracking-wider">
                                    {label}:
                                </span>
                                <span className="text-lg lg:text-xl text-white">{value}</span>
                            </div>
                        );
                    })}
                </motion.div>
